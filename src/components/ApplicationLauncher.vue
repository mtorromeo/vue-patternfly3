<template>
  <li v-bind="ouiaProps" class="applauncher-pf dropdown" :class="{'applauncher-pf-block-list': !list, open: open && !disabled}">
    <a :id="`pf-application-launcher-${$.uid}`" class="dropdown-toggle drawer-pf-trigger-icon nav-item-iconic" :class="{disabled: disabled || !$slots.default}" href="javascript:void(0)" @click.prevent.stop="toggle">
    ...
    <ul
      ...
      role="menu"
      :aria-labelledby="`pf-application-launcher-${$.uid}`"
      @click.stop="hide"
    >
      <slot />
    </ul>
